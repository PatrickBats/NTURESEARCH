...
def main():
    parser = argparse.ArgumentParser(
        '4-way texture‐prototype eval (same class & diff texture) using CLIP'
    )
    # hardcode CLIP here:
    ...
    print('[ℹ️] 4-way trials: same-class, different-texture distractors (CLIP)')
    for cls, tex_groups in ct_idxs.items():
        for tex, idx_list in tex_groups.items():
            # distractors: same class but different texture
            pool = [
                i for i, c, t in zip(all_idxs, all_classes, all_textures)
                if c == cls and t != tex
            ]
    ...
    print(f"\nOverall accuracy (CLIP, same-class diff-texture): "
          f"{total_correct}/{total_trials} ({overall:.1%})")
