...
THIS_DIR = os.path.dirname(os.path.abspath(__file__))  # …/DifferentClass_DifferentTexture
...
def main():
    parser = argparse.ArgumentParser(
        '4-way texture‐prototype eval (diff class & diff texture) using CLIP'
    )
    # hardcode CLIP baseline here:
    parser.add_argument('--model', default='clip-resnext',
                        help='(will load CLIP‐based feature extractor)')
    ...
    # ─── load dataset & CLIP model ───
    df = pd.read_csv(CSV_PATH)
    model, transform = load_model('clip-resnext',
                                  seed=args.seed,
                                  device=args.device)
    extractor = FeatureExtractor('clip-resnext', model, args.device)
    ...
    with torch.no_grad():
        for imgs, classes, textures, idxs in loader:
            ...
            feats = extractor.norm_features(feats).cpu().float()   # cast from half
    ...
    print('[ℹ️] 4-way trials: different-class, different-texture distractors (CLIP)')
    for cls, tex_groups in ct_idxs.items():
        for tex, idx_list in tex_groups.items():
            # distractors: must come from a different class *and* different texture
            pool = [i for i, c, t in zip(all_idxs, all_classes, all_textures)
                    if c != cls and t != tex]
            ...
            for _ in range(args.trials_per_texture):
                ...
                proto = (all_embs[[all_idxs.index(i) for i in same_tex]].mean(0)
                         if same_tex else all_embs[all_idxs.index(q)])
                proto = proto / proto.norm()

                distractors = random.sample(pool, 3)
                candidates  = [q] + distractors
                sims = (all_embs[[all_idxs.index(i) for i in candidates]] @ proto)
            ...
            print(f"{cls:12s} / {tex:12s}: {correct}/{args.trials_per_texture} "
                  f"({correct/args.trials_per_texture:.1%})")

    overall = total_correct / total_trials if total_trials else 0.0
    print(f"\nOverall accuracy (CLIP): {total_correct}/{total_trials} ({overall:.1%})")
