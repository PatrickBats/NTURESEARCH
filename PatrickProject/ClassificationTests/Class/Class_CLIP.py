...
def main():
    parser = argparse.ArgumentParser("4-way class-prototype eval")
    parser.add_argument('--model',           default='clip-resnext',
                        help="which CLIP model checkpoint to load")
    ...
    # 3) load data + extract all embeddings
    ...
    # 4) build maps for prototype evaluation
    idx2class  = {i:c for i,c in zip(all_idxs, all_classes)}
    idx2row    = {i:row for row,i in enumerate(all_idxs)}
    class2idxs = {}
    for i, c in idx2class.items():
        class2idxs.setdefault(c, []).append(i)

    # 5) run 4-way class prototype trials
    total_correct = 0
    total_trials  = 0
    ...
    print(f"[✅] Appended overall result to {MASTER_CSV}")
