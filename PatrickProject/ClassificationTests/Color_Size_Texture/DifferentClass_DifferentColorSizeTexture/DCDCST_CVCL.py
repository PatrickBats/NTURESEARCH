...
THIS_DIR = os.path.dirname(os.path.abspath(__file__))  # …/DifferentClass_DifferentColorSizeTexture
...
def main():
    parser = argparse.ArgumentParser("4-way CVCL prototype eval: different-class, different-color/size/texture")
    ...
    total_correct = 0
    total_trials  = 0
    print("[ℹ️] Running 4-way trials: distractors differ in class, color, size & texture")
    for meta, idx_list in tuple2idxs.items():
        cls, col, sz, tex = meta
        # must have at least 1 query and ≥3 distractors from entirely different tuples
        distractor_pool = [
