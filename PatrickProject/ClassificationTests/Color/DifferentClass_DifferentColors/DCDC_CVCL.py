...
MASTER_CSV = os.path.join(
    REPO_ROOT, 'PatrickProject', 'Chart_Generation', 'all_prototype_results.csv'
)
...
def main():
    ...
    # ─── run true baseline: distractors from *any* other image ───
    ...
    total_correct = 0
    total_trials  = 0
    print("[ℹ️] Running 4-way trials: pick one red-apple among any other distractors …")
    ...
    summary = f"\nOverall baseline accuracy: {total_correct}/{total_trials} ({overall:.1%})"
    print(summary)

    # ─── append a row to the master CSV ───
