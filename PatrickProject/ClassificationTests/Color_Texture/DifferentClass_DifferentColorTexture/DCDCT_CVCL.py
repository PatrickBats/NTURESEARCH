...
THIS_DIR = os.path.dirname(os.path.abspath(__file__))  # …/DifferentClass_DifferentColorTexture
...
def main():
    parser = argparse.ArgumentParser(
        "4-way CVCL eval: different-class, different-color & texture"
    )
    ...
    total_correct = 0
    total_trials  = 0
    print("[ℹ️] Running 4-way trials: distractors with class≠, color≠ & texture≠")
    for (cls, col, tex), idx_list in tuple2idxs.items():
        # distractors must differ in class, color AND texture
        distractor_pool = [
