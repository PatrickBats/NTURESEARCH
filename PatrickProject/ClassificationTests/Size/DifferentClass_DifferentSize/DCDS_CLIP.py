...
THIS_DIR = os.path.dirname(os.path.abspath(__file__))  # …/Size/DifferentClass_DifferentSize
...
def main():
    parser = argparse.ArgumentParser(
        "4-way size-prototype eval (diff class & diff size)"
    )
    ...
    print("[ℹ️] Running 4-way trials: pick one size‐specific sample among any other class & size distractors …")
    for cls, size_groups in cs_idxs.items():
        for size, idx_list in size_groups.items():
            # distractors: any image with class != cls AND size != size
            distractor_pool = [
                i for i, c, s in zip(all_idxs, all_classes, all_sizes)
                if c != cls and s != size
            ]
